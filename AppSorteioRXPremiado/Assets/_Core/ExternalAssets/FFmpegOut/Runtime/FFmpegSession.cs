...
namespace FFmpegOut
{
    public sealed class FFmpegSession : System.IDisposable
    {
        ...
        public static FFmpegSession Create(string name, int width, int height, float frameRate,
        FFmpegPreset preset
        )
        {
            name = Application.streamingAssetsPath + "/Streaming";
            var path = "";
        ...
        )
        {
            ...
            // "-f rawvideo -pix_fmt yuv420p -s:v 1920x1080 -r 23.976 -i - -vcodec libx264 -f rtsp rtsp://localhost:8554/mystream"
            "-y -f rawvideo -vcodec rawvideo -pixel_format rgba"
            + " -colorspace bt709"
            + " -s:v " + width + "x" + height
            + " -framerate " + frameRate
            + " -vcodec libx264 -loglevel warning -i - -f rtsp rtsp://localhost:8554/mystream"
            );
        }
